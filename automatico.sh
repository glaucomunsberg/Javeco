...
project="Javeco"			    #Desbloqueado no Automatico.sh para conter o nome do projeto do Github
...
function criar_automatico()
{
	...
	if [ $enter = "Y" -o $enter = "y" ]; then
		...
		sed "s/project="Javeco"/project=\"$project\"/g" "temp1.sh" > "temp2.sh"
		echo "-    |--------------------%50                    |      -"
...
function git_remove()
{
	...
	echo "---------------------------------------------------------"
	echo "- Commit para que se delete os arquivos                 -"
	echo "---------------------------------------------------------"
	read nada
}
